...
def find_arduino_port():
    ...
    for p in ports:
        ...
        name_l = name.lower()
        if any(s in name_l for s in ["arduino", "wch", "ch340", "acm", "usb serial"]):
            return p.device
...
def send_uart(ser, cx, cy, w, h, missing_policy="neg1"):
    if ser is None:
        return
...
def parse_args():
    ap = argparse.ArgumentParser(description="Headless детектор мишени (RPi + C270) -> USB CDC UART (Arduino)")
    ap.add_argument("--cam-index", type=int, default=None)
    ap.add_argument("--width", type=int, default=1280)
    ap.add_argument("--height", type=int, default=720)
    ap.add_argument("--fps", type=int, default=30)

    ap.add_argument("--serial", type=str, default="auto", help="'auto' или путь, например /dev/ttyACM0")
    ap.add_argument("--baud", type=int, default=115200)
    ap.add_argument("--missing-policy", choices=["neg1", "zeros", "null"], default="neg1",
                    help="Что слать при отсутствии цели: neg1 (-1,-1,-1,-1), zeros или null")
    ...
    ap.add_argument("--white-ratio",type=float, default=5.0)

    ap.add_argument("--send-every", type=int, default=1)
...
def main():
    ...
    while not stop:
        ...
        if not ok or frame is None:
            print("[Camera] Кадр не получен...", file=sys.stderr)
            time.sleep(0.05)
            continue
        ...
        if frame_id % max(1, args.send_every) == 0:
            send_uart(ser, cx, cy, wmax, hmax, missing_policy=args.missing_policy)

        now = time.time()
        if (now - last_log) >= args.log_interval:
            last_log = now
            fps_est = frame_id / max(1e-6, (now - t0))
            status = f"{cx},{cy},{wmax},{hmax}" if cx is not None else (
                     "-1,-1,-1,-1" if args.missing_policy=="neg1" else
                     "0,0,0,0" if args.missing_policy=="zeros" else
                     "null,null,null,null")
